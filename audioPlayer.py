...
class AudioPlayer:
    def __init__(self, callbacks={}, debug=0):
        ...
        self.AACDecoder = AudioDecoder.AACDecoder()
    ...
    def reset_player(self, reset_head=True):
        ...
        if reset_head:
            if self.ntracks > 0:
                ...
                self.callbacks["messages"](f"reset_player: current_track = {self.current_track} of {self.ntracks}")
    ...
    def __repr__(self):
        ...
        if self.PLAY_STATE != play_state_Stopped:
            ...
            retstring += f" InBuffer: {self.InBuffer.any():.0f}"
            retstring += f" OutBuffer: {self.OutBuffer.any():.0f}"

        return retstring

    def set_playlist(self, urllist):
        self.ntracks = len(urllist)
        ### TEMPORARY ###
        ...
        self.callbacks["messages"](f"set_playlist: current_track = {self.current_track} of {self.ntracks}")
    ...
    def play(self):
        ...
        if self.PLAY_STATE == play_state_Stopped:
            ...
            self.read_phase = read_phase_start
            self.trackReader = self.start_track(self.current_track)
            ...
            self.callbacks["messages"](f"Playing URL {self.playlist[self.current_track]}")

        elif self.PLAY_STATE == play_state_Paused:
            # print(f"Un-pausing URL {self.playlist[self.current_track]}")
            self.callbacks["messages"](f"Un-pausing URL {self.playlist[self.current_track]}")
            self.PLAY_STATE = play_state_Playing
    ...
    def pause(self):
        if self.PLAY_STATE == play_state_Playing:
            self.mute_audio()
            self.callbacks["messages"](f"Pausing URL {self.playlist[self.current_track]}")
    ...
    def advance_track(self, increment=1):
        ...
        self.next_track = self.set_next_track()
        self.callbacks["messages"](f"advance track: current_track = {self.current_track}. next_track = {self.next_track}")
        print(self)
    ...
    def start_track(self, trackno, offset=0, port=80):
        ...
        conn = socket.socket()
        self.callbacks["messages"](f"start_track: Start reading track {self.track_being_read} of {self.ntracks}")
    ...
    def end_track(self):
        ...
        if self.track_being_read + 1 < self.ntracks:
            ...
            self.DEBUG and print("Track read start")
            self.trackReader = self.start_track(self.track_being_read + 1)
            self.callbacks["messages"](f"end_track: Finished reading track {self.track_being_read} of {self.ntracks}")
        else:
        ...
        if self.read_phase == read_phase_start:
            ...
            try:
                ...
                if data is not None:
                    ...
                    # Start the decode loop once we have about 16kB in the InBuffer. No point starting decoding too early or the decoder can fail with insufficient data
                    if self.InBuffer.any() > 16384:
                ...
                self.start_track(self.track_being_read, self.current_track_bytes_read)
    ...
    def decode_chunk(self, timeout=10):
        ...
        if self.decode_phase == decode_phase_trackstart:
            ...
            self.DEBUG and print(f"Track {self.current_track} decode start")
            self.current_track_bytes_decoded_out = 0
        ...
        if self.decode_phase == decode_phase_inheader:
            ...
            if self.TrackInfo[0][1] == format_AAC:
                ...
                print(self.AACDecoder.write_free(), self.InBuffer.any())
        ...
        while True:
            ...
            if self.TrackInfo[0][1] == format_AAC:
                ...
                while self.AACDecoder.write_free() >= 188 and self.InBuffer.any() >= 188:
                    ...
                    if self.DEBUG > 1:
                        print("Parsed:", parsedLength, self.current_track_bytes_parsed_in, self.current_track_bytes_parsed_out)
            ...
            if len(self.DecodeInfo) > 0:
                if self.current_track_bytes_decoder_in == self.DecodeInfo[0]:  # We have finished decoding the current track.
                    self.DEBUG and print(f"Track {self.current_track} decode end")
                    ...
                    if self.current_track + 1 < self.ntracks:  # Start decode of next track
                        self.callbacks["messages"](
                            f"decode_chunk: Finished decoding track {self.current_track} of {self.ntracks}"
                        )
                        self.current_track += 1
                        self.next_track = self.set_next_track()
                        self.decode_phase = decode_phase_trackstart
    ...
    def play_chunk(self):
        ...
        if len(self.PlayLength) > 0:
            ...
            if self.current_track_bytes_played + BytesToPlay >= self.PlayLength[0]:
                ...
                self.callbacks["messages"]("play_chunk: Finished playing track")
